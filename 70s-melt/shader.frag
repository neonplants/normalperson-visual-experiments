...
void main()
{
    ...
    // Custom color palette: light blue, orange, salmon-red
    vec3 lightBlue = vec3(0.53, 0.81, 0.92);   // #87CEEB
    vec3 orange = vec3(1.0, 0.55, 0.0);         // Vivid orange
    vec3 salmonRed = vec3(0.98, 0.45, 0.45);    // Salmon-red

    // Create pattern values from the distorted coordinates
    float t1 = 0.5 * sin(3.0 * p.x) + 0.5;
    float t2 = 0.5 * sin(3.0 * p.y) + 0.5;
    float t3 = 0.5 * sin(p.x + p.y) + 0.5;

    // Blend between the three colors
    vec3 col = mix(lightBlue, orange, t1);
    col = mix(col, salmonRed, t2 * 0.6);
    col = mix(col, lightBlue * 1.2, t3 * 0.3);
